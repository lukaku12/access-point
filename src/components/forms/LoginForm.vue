  ...
  <div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4" autocomplete="on">
      ...
      <BaseInput
        ...
        name="password"
        @blur="
          validateField('authKey', formData.authKey, validationRules.authKey)
        "
        required
      />
      <button
...
const validationRules = {
  ip: {
    ...
    format: (v: string) =>
      /^(\d{1,3}\.){3}\d{1,3}$/.test(v) || 'Invalid IP address format'
  },
  port: {
  ...
  authKey: {
    ...
    minLength: (v: string) =>
      v.length >= 6 || 'Auth key must be at least 6 characters'
  }
};
...
const handleSubmit = async () => {
  try {
    ...
    const isPortValid = validateField(
      'port',
      formData.port,
      validationRules.port
    );
    const isAuthValid = validateField(
      'authKey',
      formData.authKey,
      validationRules.authKey
    );
