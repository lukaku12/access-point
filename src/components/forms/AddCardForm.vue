...
const validateForm = () => {
    validateField('cardNumber', searchQuery.value, {
        ...
        minLength: (v) => v.length >= 3 || 'Card number must be at least 3 characters'
    })
    ...
    if (formData.value.end_date) {
        validateField('endDate', formData.value.end_date, {
            greaterThanStart: (v) => new Date(v) > new Date(formData.value.start_date) || 'End date must be after start date'
        })
    }
...
<template>
    <form @submit.prevent="handleSubmit" class="space-y-8 max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200/60 shadow-lg shadow-gray-200/40 hover:shadow-xl hover:shadow-gray-200/60 transition-all duration-200">
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    <h4 class="text-lg font-semibold text-gray-900">Card Details</h4>
                    <button type="button" @click="loadUnregisteredCards"
                ...
                <!-- Card Number Input -->
                <div class="relative">
                    <BaseInput v-model="searchQuery" label="Card Number" placeholder="Type or select card number"
                        help-text="Start typing to search or select from available cards"
                        :error="touched?.cardNumber ? errors?.cardNumber : ''" required @focus="isOpen = true"
                        @blur="() => { closeDropdown(); handleBlur('cardNumber') }" />

                    <div v-if="isOpen && filteredCards.length > 0"
                        class="absolute z-10 mt-1 w-full bg-white rounded-lg border border-gray-200 shadow-lg max-h-60 overflow-auto">
                        <div v-for="card in filteredCards" :key="card.card_number" @mousedown="handleCardSelect(card)"
                ...
                <BaseInput v-model="formData.personal_id" label="Personal ID" placeholder="Enter personal ID" />
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200/60 shadow-lg shadow-gray-200/40 hover:shadow-xl hover:shadow-gray-200/60 transition-all duration-200">
            ...
            <div class="grid grid-cols-2 gap-8">
                <BaseInput v-model="formData.start_date" type="date" label="Start Date" required
                    :error="touched?.startDate ? errors?.startDate : ''" @blur="() => handleBlur('startDate')" />
                <BaseInput v-model="formData.end_date" type="date" label="End Date"
                    :error="touched?.endDate ? errors?.endDate : ''" @blur="() => handleBlur('endDate')" />
            </div>
        </div>
